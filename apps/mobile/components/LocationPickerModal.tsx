...
const DEFAULT_LAT = 19.7515; // Maharashtra center
...
const CIRCLE_STYLE: L.CircleOptions = {
  color: '#4A8C2A',
  fillColor: '#2D5A1B',
  fillOpacity: 0.1,
  weight: 1.5,
...
export default function LocationPickerModal({ isOpen, current, isMr, onApply, onClose }: Props) {
  ...
  const radiusRef  = useRef(radius); // always-current ref for map callbacks
  ...
  // Update circle radius live as slider moves
  useEffect(() => {
  ...
  // Place / move circle on map
  const placeCircle = (lat: number, lng: number, r: number) => {
    if (!mapRef.current) return;
    if (circleRef.current) {
      circleRef.current.setLatLng([lat, lng]);
      circleRef.current.setRadius(r * 1000);
    } else {
      circleRef.current = L.circle([lat, lng], { ...CIRCLE_STYLE, radius: r * 1000 }).addTo(mapRef.current);
    }
  };

  // Reverse-geocode and update UI + circle
  const geocodeLatLng = async (lat: number, lng: number) => {
    setGeocoding(true);
    setPinLat(lat);
    setPinLng(lng);
    placeCircle(lat, lng, radiusRef.current);
    try {
      ...
      const full = [name, district, state].filter(Boolean).join(', ');
      setPinLabel(full);
      setPinLabelMr(full);
    } catch {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      // Draw circle if we already have a pin
      if (pinLat && pinLng) {
      ...
      {isOpen && (
        <>
          <style>{`
            ...
            .agri-slider::-moz-range-track { height:6px; border-radius:99px; background:rgba(245,240,232,0.13); }
            .agri-slider::-moz-range-progress { height:6px; border-radius:99px; background:#2D5A1B; }
            .agri-slider::-moz-range-thumb { width:28px; height:28px; border-radius:50%; background:#D4C4A0; border:3px solid #0A1A0A; box-shadow:0 0 0 2.5px #2D5A1B; cursor:pointer; }
            .leaflet-control-attribution { font-size:8px !important; opacity:0.35; background:rgba(10,26,10,0.55) !important; color:#F5F0E8 !important; }
          ...
          {/* Sheet — flex col, fixed 90dvh, overflow hidden */}
          <motion.div
            drag="y"
            ...
            style={{ background: '#0F1F0F', border: '1px solid rgba(245,240,232,0.09)', height: '90dvh', overflow: 'hidden' }}
          ...
          >
            ...
            {/* Map — fixed 220px so remaining content always fits */}
            <div className="relative flex-shrink-0" style={{ height: 220 }}>
              ...
              <button
                ...
                style={{
                  background: '#0F1F0F',
                  border: '1px solid rgba(74,140,42,0.6)',
              ...
              >
                ...
                <span style={{ fontSize: '12px', fontWeight: 500, color: '#D4C4A0', letterSpacing: '0.02em' }}>
                  ...
                  {geocoding ? (
                    ...
                    <p style={{ fontSize: '14px', fontWeight: pinLabel ? 400 : 300, color: pinLabel ? '#F5F0E8' : 'rgba(245,240,232,0.3)', lineHeight: 1.4 }}>
                      {pinLabel
                        ...
                        : (isMr ? 'नकाशावर टाचणी ठेवा किंवा माझे स्थान दाबा' : 'Tap on map or press My Location')}
                    </p>
                  )}
                </div>
              </div>
